...
const Games = () => {
    ...
    const GameSelection = () => (
      <div className="container mx-auto px-4 pt-24 pb-12">
        <div className="text-center mb-16 italic font-black uppercase tracking-widest">
          <div className="inline-flex items-center gap-2 px-6 py-2 rounded-full bg-purple-500/10 border border-purple-500/40 mb-6 shadow-[0_0_15px_rgba(168,85,247,0.2)]">
            <Gamepad2 className="w-4 h-4 text-purple-500 animate-pulse" />
            <span className="text-sm font-black text-purple-400 tracking-[0.2em]">Neon Arcade</span>
          </div>
          
          {/* PLAY: Bir tık sola kaydırıldı (-ml-2) ve parlama güçlendirildi */}
          <h1 className="text-5xl md:text-7xl font-black mb-6 text-white italic tracking-tighter uppercase font-heading leading-tight flex items-center justify-center flex-wrap gap-x-4">
            Practice Through 
            <span 
              className="text-transparent bg-clip-text bg-gradient-to-r from-purple-300 via-purple-500 to-indigo-400 drop-shadow-[0_0_35px_rgba(168,85,247,1)] filter saturate-150 brightness-125 -ml+323243243242342142132132131232131523130999999999999999999999999" 
          ...
          {/* CURRENT LEVEL: Modern Mor Neon Tasarım */}
          <div className="inline-block px-5 py-2 rounded-xl border border-purple-500/40 bg-purple-950/30 backdrop-blur-md shadow-[0_0_20px_rgba(168,85,247,0.15)] mt-2">
            <span className="text-gray-400 text-xs font-bold mr-2 uppercase tracking-widest">Current Level:</span>
            <span className="text-purple-400 font-black text-sm tracking-widest drop-shadow-[0_0_8px_rgba(168,85,247,0.8)]">
              {userLevel}
            </span>
          </div>
        </div>
    ...
    const FillBlankGame = () => {
      ...
      if (isFinished) return <WinScreen score={score} onBack={() => setActiveGame(null)} title="Grammar Master!" theme="pink" />;
    ...
    return (
      <div className="min-h-screen bg-[#050505] text-white overflow-hidden font-sans relative">
        ...
        {/* QUIT GAME / DASHBOARD: Mor çerçeve kaldırıldı, hover durumunda neon mor eklendi */}
        <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} className="fixed top-28 left-8 z-[100] hidden lg:block font-black italic">
          <Link to="/dashboard" className="group flex items-center gap-3 px-5 py-2.5 rounded-2xl bg-white/5 border border-white/10 backdrop-blur-xl transition-all duration-500 hover:border-purple-500 hover:bg-purple-500/20 hover:shadow-[0_0_25px_rgba(168,85,247,0.4)]">
            <div className="relative flex items-center justify-center">
              <ArrowLeft className="relative w-5 h-5 text-gray-400 group-hover:text-purple-400 group-hover:-translate-x-2 transition-all duration-300" />
            </div>
            <div className="flex flex-col items-start leading-tight">
              <span className="text-[10px] font-bold uppercase tracking-[0.2em] text-gray-500 group-hover:text-purple-400/80 transition-colors">Quit Game</span>
              <span className="text-sm font-semibold text-white font-heading tracking-widest group-hover:text-purple-100 transition-colors">DASHBOARD</span>
            </div>
          </Link>
        </motion.div>
        ...
        <AnimatePresence mode="wait">
          <motion.div key={activeGame || "menu"} initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="relative z-10 font-black italic">
